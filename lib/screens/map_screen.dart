...
class _ValveMapItem {
  ...
  factory _ValveMapItem.fromJson(Map<String, dynamic> json) {
    return _ValveMapItem(
      ...
      zone: json['zone'] as String? ?? 'North Field',
...
class _MapScreenState extends State<MapScreen> {
  ...
  Future<void> _loadMapData() async {
    try {
      ...
      var raw = prefs.getString(_valvesKey);
      raw ??= prefs.getString(_legacyValvesKey);
      ...
      List<_ValveMapItem>? loadedValves;
      if (raw != null) {
        try {
          final decoded = jsonDecode(raw);
          if (decoded is List) {
            loadedValves = decoded
                .whereType<Map>()
      ...
      if (raw != null && prefs.getString(_valvesKey) == null && loadedValves != null) {
        await _saveMapData();
      }
    } catch (_) {
      if (mounted) {
          ...
          children: [
            ...
            Expanded(
              child: Column(
                ...
                children: [
                  Text('Valve ID: ${valve.id}', style: const TextStyle(fontWeight: FontWeight.bold)),
...
class _MapSurfaceState extends State<_MapSurface> {
  ...
  String _locationStatus = 'Locating phone...';
  ...
  Widget build(BuildContext context) {
    ...
    final center = _phoneLocation ?? _defaultCenter;
    return ClipRRect(
        ...
        child: Stack(
          children: [
            FlutterMap(
              ...
              options: MapOptions(initialCenter: center, initialZoom: 15, minZoom: 3, maxZoom: 19),
              children: [
                TileLayer(
            ...
            Positioned(
              ...
              child: Column(
                children: [
                  ...
                  _MapButton(
                    icon: Icons.add,
                    onPressed: () => _mapController.move(_mapController.camera.center, (_mapController.camera.zoom + 1).clamp(3, 19)),
                  ),
                  _MapButton(
                    icon: Icons.remove,
                    onPressed: () => _mapController.move(_mapController.camera.center, (_mapController.camera.zoom - 1).clamp(3, 19)),
