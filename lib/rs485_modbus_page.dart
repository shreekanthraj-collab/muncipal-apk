...
class _Rs485ModbusPageState extends State<Rs485ModbusPage> {
  static const _selectionKey = 'municipal_rs485_selection_v1';
  ...
  Future<void> _loadSelection() async {
    ...
    try {
      ...
      // Keep the frozen defaults if saved selection data is invalid.
    }
  }
  ...
  Future<void> _addValve() async {
    ...
    final wardController = TextEditingController(text: ward);
        ...
        actions: [
          ...
          FilledButton(
            onPressed: () {
              ...
              final zoneNo = zoneController.text.trim();
              final wardNo = wardController.text.trim();
              if (id.isEmpty || zoneNo.isEmpty || wardNo.isEmpty) return;
    ...
    final newValve = result['valve']!;
    final newZone = result['zone']!;
    final newWard = result['ward']!;

    setState(() {
      if (!valveIds.contains(newValve)) {
        valveIds = [...valveIds, newValve];
      }
      valve = newValve;
      zone = newZone;
      ward = newWard;
    ...
    if (mounted) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('$newValve saved — Zone $newZone, Ward $newWard')),
      );
    }
  }
